...
| Reliability | 86 | Stop deadlines, cancellation, controller locking, migrations, and task transitions are tested. Live approval proofs cover revision rejection, advertised baseline execution/cleanup, runner-compatible success, and missing-tool failure with no saved approval. | Crash-recovery and concurrent-scheduler integration tests. |
...
| Test coverage | 95 | All 183 tests and package/install checks pass on the host; the complete suite also passes in the real no-network Docker VM. Coverage now includes privacy-aware preview, semantic setup reuse, no-work status, portfolio scoring, command preflight, candidate binding, publication, and cleanup. | Preserve this gate while adding interrupted-controller and scheduler integration cases. |
| Maintainability | 64 | Repo trust persistence is now isolated in a directly tested module alongside publication, evidence, reporting, and selection policy. The main controller still owns setup, queue construction, and dispatch. | Continue splitting setup, scanning, queueing, dispatch, and lifecycle into tested modules. |
