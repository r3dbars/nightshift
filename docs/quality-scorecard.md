...
| Task selection | 62 | Two live July 12 repo rehearsals proved complete deterministic coverage evidence now outranks broad missions, pinned failed CI outranks coverage leads, and weak ungrounded work is rejected before promotion. | Demonstrate high accepted-outcome rates across varied live GitHub signals and repos. |
...
| Test coverage | 91 | The full discovered suite has 124 tests covering evidence-first task ordering, relevant test-file ranking, retry correction commands, Windows evidence retry boundaries, reporting, setup, discovery, and copied installs. | Real provider, interrupted-controller, scheduler, and GitHub-write integration tests. |
