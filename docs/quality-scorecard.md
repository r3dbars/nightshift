...
and portfolio morning briefs; the current package gate passes 419 tests on this
...
| Efficiency | 92 | Weak portfolio work can be filtered to zero tokens. Live explicit-goal runs produced verified candidates in about 3,204 and 3,363 estimated local tokens, and the fresh BetterFeedback recovery produced a verified local draft in 3,480 tokens after a 3,516-token rejected attempt. The fresh three-repo cycle spent 13,940 tokens for one verified local draft while skipping five weak signals before model calls. Blank-home unchanged-revision repeats ran with zero model calls and zero-token reports. A fresh cross-repo Symphony run then skipped ten weak signals before dispatch, used zero model tokens, and still produced a truthful brief without touching its pre-existing dirty checkout. This is still a small sample, not a full-run accepted-outcome bound. | Prove strong tokens-per-accepted-outcome across full overnight runs with revision caching. |
