...
| Useful output | 91 | Explicit Night Shift goals produced source-grounded candidates, isolated `VERIFIED_DRAFT` results, and accepted merged outcomes in Night Shift PRs #111 and #120. Two fresh BetterFeedback two-lane runs produced grounded test candidates with exact files, evidence, verification, and review choices; a fresh local retry then produced a one-file `VERIFIED_DRAFT` in 3,480 tokens after the prior attempt was correctly rejected for a false JavaScript rounding assertion. A separate BetterFeedback Windows run produced a locally verified test draft and draft PR #491, but hosted Vercel statuses failed and human usefulness review remains pending. A fresh three-repo run still had no model-ready tasks, so repeated accepted usefulness across varied repos remains unproven. | Produce independently complete accepted patches across varied healthy overnight runs without duplicate churn. |
...
| Repository prioritization | 90 | Live 30-day ranking reduces stale branch and green-draft noise, guarantees the explicit primary repo a selected slot, accepts validated user `owner/repo` priorities, and now applies a real saved useful vote to the canonical GitHub repo identity. In the three-repo replay, Night Shift received `outcome_adjustment: 50`, moved ahead of Transcripted, and the brief explained why. Consistent accepted-value correlation across more runs is not proven. | Measure that selected repos consistently match accepted user value across repeated overnight runs. |
| GitHub usefulness | 92 | PR, issue, failed-workflow, and recent-repo signals feed queues. Live ranking distinguishes actionable failed/review work from stale branch and green-draft volume; failed Status Contexts now route to review and exact failed hosted checks appear in the publication artifact and morning brief. Night Shift PRs #111 and #120 passed Ubuntu/macOS CI and merged. BetterFeedback draft PR #491 remains safely open from an exact SHA with two explicitly failed Vercel checks. Repeated accepted outcomes across owned repos remain missing. | Correlate selected work to accepted PR/issue outcomes across owned repos, including a passing hosted draft. |
...
| Multi-repo operation | 86 | Portfolio cycles, stop limits, controller lock, and explicit priority selection exist. A repeated live cycle visited three owned repos, completed three child batches with no duplicate ledger, and produced an honest zero-token closeout because no task met the evidence bar. Useful verified outcomes across mixed-language repos remain unproven. | Test useful verified outcomes across real mixed-language portfolio runs. |
...
| Efficiency | 88 | Weak portfolio work can be filtered to zero tokens. Live explicit-goal runs produced verified candidates in about 3,204 and 3,363 estimated local tokens, and the fresh BetterFeedback recovery produced a verified local draft in 3,480 tokens after a 3,516-token rejected attempt. The three-repo Normal scan spent no model tokens. This is still a small sample, not a full-run accepted-outcome bound. | Prove strong tokens-per-accepted-outcome across full overnight runs with revision caching. |
...
| Morning UX | 88 | Empty grounded runs give an honest retry message. Single-repo and portfolio briefs now lead with a friendly short version, tell the reader to start with one choice, show copy-ready useful/not-useful commands, concrete note wording, the current repo learning snapshot, optional elapsed time from viewed brief to vote, and hosted-check state when a draft PR was opened; focused regression tests protect the copy. A fresh real two-lane BetterFeedback brief displayed the short version, two grounded choices, exact evidence, verification, proof, handoff, and one-action feedback commands. User comprehension and review-effort studies are still missing. | Validate morning comprehension and review effort across varied real overnight outcomes. |
