...
| Task selection | 80 | A second fresh three-repo rehearsal skipped 108 weak signals and spent zero tokens. Normal mode now rejects multi-item tracker issues as well as coverage-index guesses; single-action issues remain eligible and Afterburner keeps the broad exploration path. | Demonstrate high accepted-outcome rates across varied live GitHub signals and repos. |
...
| Efficiency | 78 | Consecutive fresh three-repo rehearsals reduced model use from about 20,782 to 6,031 and then zero tokens by filtering coverage guesses and multi-item trackers before dispatch. Useful-output efficiency is still unproven because the latest run correctly did no work. | Prove strong tokens-per-accepted-outcome across full overnight runs with revision caching. |
...
| Test coverage | 95 | All 163 tests and package/install checks pass on the host; the prior complete gate also passed in the real no-network Docker VM. Coverage now includes tracker actionability, Normal/Afterburner policy, strict override intent, shared status parsing, runner, publication, and cleanup paths. | Preserve this gate while adding interrupted-controller and scheduler integration cases. |
