...
and portfolio morning briefs; the current package gate passes 438 tests on this
...
438 tests.
The fresh local outcome replay produced one verified draft at 3,428 estimated
tokens and a clean three-repo Mac-only replay visited BetterFeedback,
...
| Multi-repo operation | 93 | Portfolio cycles, stop limits, controller lock, explicit privacy routing, and explicit priority selection exist. The latest clean-main Mac-only cycle discovered three owned repos, prepared all three checkouts, ranked BetterFeedback first for three failed checks, spent zero tokens on two no-work repos, and completed one safely rejected Night Shift candidate without duplicate ledgers. Earlier live cycles include one verified local Night Shift draft. PR #188 then proved that numbered choices from the parent portfolio brief resolve to the requested child source without losing the portfolio context. Useful verified outcomes across more varied mixed-language portfolio runs remain unproven. | Test useful verified outcomes across repeated real mixed-language portfolio runs. |
