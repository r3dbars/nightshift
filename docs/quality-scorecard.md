...
| Test coverage | 94 | The full discovered suite has 157 tests, including named/default Colima context and socket validation plus writable-tmpfs verification from a read-only source mount. A real Docker run also passed the 9-test publisher safety suite before opening draft PR #24. | Interrupted-controller, scheduler, broader Docker/Linux providers, and overnight GitHub-write integration tests. |
...
| Portability | 90 | The immutable runner now builds and verifies a real draft through Docker in a Colima macOS VM; prior ARM64 Podman proof remains valid, while current Podman restart behavior is unstable. | Prove Linux installation and provider restart/recovery across fresh machines. |
