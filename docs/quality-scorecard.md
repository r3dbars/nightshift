...
| Test coverage | 94 | The full discovered suite has 153 tests covering both patch lanes plus draft publication owner/origin binding, default-branch ancestry, approved-file staging, replay suppression, collision refusal, draft verification, ambiguous-push cleanup escalation, and morning warning propagation. GitHub-write success is not yet integration-proven. | Interrupted-controller, scheduler, Docker-provider, and successful GitHub-write integration tests. |
