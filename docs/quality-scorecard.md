...
| Test coverage | 95 | All 168 tests and package/install checks pass on the host; the prior complete gate also passed in the real no-network Docker VM. Coverage now includes one-citation issue enforcement, intent-polarity rejection, actionable ranking, pinned correction, runner, publication, and cleanup paths. | Preserve this gate while adding interrupted-controller and scheduler integration cases. |
