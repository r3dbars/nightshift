...
| Efficiency | 62 | The live Windows repair used about 440 worker tokens across one draft and one format correction to reach a deterministically accepted outcome; broad overnight efficiency remains poor. | Prove strong tokens-per-accepted-outcome across full overnight runs with revision caching. |
...
| Patch autonomy | 88 | The live Windows model generated a real one-file repair; one strict format retry plus hunk recount kept validation bounded, and Podman promoted it only after failing-before/passing-after proof. | Prove model-generated isolated patches across diverse real repositories. |
...
| Test coverage | 94 | The full discovered suite has 141 tests covering exact header-path binding, complete-diff validation, multi-file correction scope, STOP-aware retry bounds, and hunk recount; live Windows and Podman proof exercises the complete repair path. | Interrupted-controller, scheduler, Docker-provider, and GitHub-write integration tests. |
