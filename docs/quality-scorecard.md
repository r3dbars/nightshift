...
| Useful output | 45 | Three fresh real Draft replays selected open issue #38 and the exact disabled flag at `ParakeetEngine.swift:51`; one produced a validator-clean MAYBE while two were correctly rejected for broader unsupported claims. No approved repo profile existed, so no repair was attempted. | Varied healthy overnight runs produce independently verified useful outcomes without duplicate churn. |
...
| Efficiency | 75 | Weak three-repo work can be filtered to zero tokens, but each Draft issue replay used about 7,700 Windows tokens across an initial review and correction; only one of three final-form replays passed evidence validation. | Prove strong tokens-per-accepted-outcome across full overnight runs with revision caching. |
| Evidence quality | 92 | The real Draft issue candidate was pinned to a commit, limited to exact symbol-matched files, and accepted only after its final output cited the strongest exact source line with zero validator reasons. It remains MAYBE because source evidence is not execution proof. | Exercise reproduction evidence across varied real repositories. |
...
| Test coverage | 95 | All 165 tests and package/install checks pass on the host; the prior complete gate also passed in the real no-network Docker VM. Coverage now includes actionable issue ranking, pinned Windows correction, tracker policy, runner, publication, and cleanup paths. | Preserve this gate while adding interrupted-controller and scheduler integration cases. |
