...
| Test coverage | 94 | The full discovered suite has 156 tests, including strict Colima context/socket validation and writable-tmpfs verification from a read-only source mount. A real Docker run also passed the 9-test publisher safety suite before opening draft PR #24. | Interrupted-controller, scheduler, broader Docker/Linux providers, and overnight GitHub-write integration tests. |
