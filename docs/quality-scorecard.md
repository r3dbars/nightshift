...
and portfolio morning briefs; the current package gate passes 414 tests.
...
| Useful output | 93 | Explicit Night Shift goals produced source-grounded candidates, isolated `VERIFIED_DRAFT` results, and accepted merged outcomes in Night Shift PRs #111 and #120. Three fresh BetterFeedback replays produced grounded test candidates with exact files, evidence, verification, and review choices; the newest explicit goal produced one behaviorally bounded `MAYBE` while rejecting three weaker claims. A fresh local retry produced a one-file `VERIFIED_DRAFT` in 3,480 tokens after the prior attempt was correctly rejected for a false JavaScript rounding assertion. A fresh three-repo portfolio cycle then produced another verified Night Shift draft while safely rejecting an unproven BetterFeedback patch and keeping a Transcripted candidate as MAYBE. A separate BetterFeedback Windows run produced a locally verified test draft and draft PR #491, but hosted Vercel statuses failed and human usefulness review remains pending. Repeated accepted usefulness across varied repos remains unproven. | Produce independently complete accepted patches across varied healthy overnight runs without duplicate churn. |
| Task selection | 94 | Deterministic ranking skips trackers and ranks live failures/issues. Automatic Python coverage now prefers complete owner-aware AST gaps over textual absence, stays scoped across same-name class collisions, and a live explicit goal reached a verified draft. Three fresh BetterFeedback runs selected bounded TypeScript test gaps from invocation evidence; the newest explicit goal rejected stale-export and unsupported-negative claims before keeping one behaviorally grounded `MAYBE`. TypeScript and guarded Swift coverage can become executable only with complete invocation evidence; unreachable Swift modules and incomplete indexes remain analysis-only. | Demonstrate high accepted-outcome rates across varied clean live GitHub runs and repos. |
| Repository prioritization | 93 | Live 30-day ranking reduces stale branch and green-draft noise, guarantees the explicit primary repo a selected slot, accepts validated user `owner/repo` priorities, recognizes failed Status Contexts, and applies a real saved useful vote to the canonical GitHub repo identity. Two fresh three-repo cycles ranked BetterFeedback first for recent failing checks, Transcripted second for active work, and Night Shift third as the current project; both morning briefs explained each choice. Consistent accepted-value correlation across more runs is not proven. | Measure that selected repos consistently match accepted user value across repeated overnight runs. |
| GitHub usefulness | 93 | PR, issue, failed-workflow, and recent-repo signals feed queues. Live ranking distinguishes actionable failed/review work from stale branch and green-draft volume; failed Status Contexts now route to review and exact failed hosted checks appear in the publication artifact and morning brief. The fresh portfolio cycle used those signals to select BetterFeedback first and produced a verified local Night Shift draft from current repo evidence. Night Shift PRs #111 and #120 passed Ubuntu/macOS CI and merged. BetterFeedback draft PR #491 remains safely open from an exact SHA with two explicitly failed Vercel checks. Repeated accepted hosted outcomes across owned repos remain missing. | Correlate selected work to accepted PR/issue outcomes across owned repos, including a passing hosted draft. |
| Portfolio discovery | 93 | Fresh authenticated discovery selected and cached three owner-validated private repos, prepared all three checkouts, rejected symlink/path-escape/foreign-owner cases, and supports validated explicit priorities plus quiet hours. The live cycle completed a child batch for each discovered repo and kept the current project in the portfolio. Varied-account and repeated accepted-value behavior remains unproven. | Prove varied-account behavior and correlate discovery to accepted outcomes. |
...
| Multi-repo operation | 92 | Portfolio cycles, stop limits, controller lock, and explicit priority selection exist. Two fresh live cycles discovered three owned repos and completed three child batches each with no duplicate ledger; one produced a verified local Night Shift draft, while the newer current-main replay kept two BetterFeedback candidates unproven and correctly recorded no-work for Transcripted and Night Shift. PR #188 then proved that numbered choices from the parent portfolio brief resolve to the requested child source without losing the portfolio context. Useful verified outcomes across more varied mixed-language portfolio runs remain unproven. | Test useful verified outcomes across repeated real mixed-language portfolio runs. |
...
| Efficiency | 91 | Weak portfolio work can be filtered to zero tokens. Live explicit-goal runs produced verified candidates in about 3,204 and 3,363 estimated local tokens, and the fresh BetterFeedback recovery produced a verified local draft in 3,480 tokens after a 3,516-token rejected attempt. The fresh three-repo cycle spent 13,940 tokens for one verified local draft while skipping five weak signals before model calls. A new blank-home replay then ran two unchanged-revision `start --yes --once` cycles, both with zero model calls and zero-token reports. This is still a small sample, not a full-run accepted-outcome bound. | Prove strong tokens-per-accepted-outcome across full overnight runs with revision caching. |
