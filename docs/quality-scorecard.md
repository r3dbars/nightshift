...
| Task selection | 74 | A fresh three-repo rehearsal skipped 107 weak signals before dispatch, spent zero tokens on Night Shift's coverage-index guesses, and evaluated only two maintainer-filed issues. Coverage-only exploration remains available through Afterburner or an explicit coverage goal. | Demonstrate high accepted-outcome rates across varied live GitHub signals and repos. |
...
| Efficiency | 72 | The fresh three-repo rehearsal reduced model use from about 20,782 to 6,031 tokens, a 71% reduction, by rejecting coverage-only guesses before dispatch. The two remaining issue reviews were rejected and useful-output efficiency is still unproven. | Prove strong tokens-per-accepted-outcome across full overnight runs with revision caching. |
...
| Morning UX | 68 | The portfolio brief now labels MAYBE-only work as unproven instead of repeating a model claim, and the fresh three-repo rehearsal ended with a concise per-repo no-outcome summary. | Validate morning comprehension and review effort across varied real overnight outcomes. |
...
| Test coverage | 95 | All 161 tests and package/install checks pass on the host; the prior complete gate also passed in the real no-network Docker VM. Coverage now includes Normal/Afterburner signal policy, strict override intent, and shared status parsing in addition to runner, publication, and cleanup paths. | Preserve this gate while adding interrupted-controller and scheduler integration cases. |
...
| Honesty about proof | 94 | Worker claims stay separate from deterministic and manual proof. A live mismatch where the terminal printed GREEN while the saved portfolio brief said YELLOW is fixed and replayed: both now report YELLOW, and unproven child claims are not repeated as findings. | Preserve this distinction through real draft-PR and cloud-handoff flows. |
