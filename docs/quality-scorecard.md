...
| Minimal setup | 78 | Live interactive and `--yes` setup require no repo path, URL, model name, or GitHub selection. A real blank-home `trust-repo` preview detected Draft ownership and verification automatically; a disposable owned-repo apply needed one explicit consent and never edited the repo. | Repeat on fresh macOS/Linux users and prove clear recovery when no eligible repo or AI is available. |
...
| Reliability | 84 | Stop deadlines, process cancellation, single-controller lock, migrations, and task transitions are tested. Live external-approval proofs now cover both pre-execution revision rejection and successful advertised baseline execution/cleanup. | Crash-recovery and concurrent-scheduler integration tests. |
...
| Test coverage | 95 | All 176 tests and package/install checks pass on the host; the complete suite also passed in the real no-network Docker VM during the advertised-candidate baseline proof. Coverage includes positive/negative candidate binding, typed external-profile metadata, spoof/tamper and symlink rejection, runner, publication, and cleanup paths. | Preserve this gate while adding interrupted-controller and scheduler integration cases. |
