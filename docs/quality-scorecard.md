...
and portfolio morning briefs; the current package gate passes 493 tests on this
Mac (the isolated runner records one skipped installed-Claude compatibility
test when the installed CLI lacks the legacy flag).
...
493 tests, with one installed-Claude compatibility check skipped in the
isolated runner because the CLI lacks the legacy flag. PRs #254, #255, #256, #257, #258, #259, #261, #263, #265, #267, #269, #271, #272, #273, and #274 now reject out-of-pack review citations,
require exact pinned revisions before a cloud review can be sent, persist
review outcomes atomically, link exact valid handoff reviews to morning feedback, and reconcile
feedback recorded before a later valid review without creating a duplicate vote. The
feedback linkage proof is recorded in
[`2026-07-14-feedback-review-linkage.md`](proofs/2026-07-14-feedback-review-linkage.md).
PR #267 also prefilters non-executable automatic coverage drafts before model calls; its
live Transcripted proof is recorded in
[`2026-07-14-coverage-prefilter.md`](proofs/2026-07-14-coverage-prefilter.md).
The bounded Python source-evidence replay is recorded in
[`2026-07-14-bounded-python-source-evidence.md`](proofs/2026-07-14-bounded-python-source-evidence.md).
The validator and log-noise replay is recorded in
[`2026-07-14-validator-and-log-noise-proof.md`](proofs/2026-07-14-validator-and-log-noise-proof.md).
The current-main runner recovery and three-repository portfolio
...
[`2026-07-14-publication-ledger-reconciliation.md`](proofs/2026-07-14-publication-ledger-reconciliation.md).
The friendly missing-agent handoff preflight proof is recorded in
[`2026-07-14-friendly-handoff-preflight.md`](proofs/2026-07-14-friendly-handoff-preflight.md).
The legacy publication-ledger reconciliation proof is recorded in
[`2026-07-14-legacy-publication-reconciliation.md`](proofs/2026-07-14-legacy-publication-reconciliation.md).
The outcome-aware learning replay is recorded in
[`2026-07-14-outcome-aware-learning.md`](proofs/2026-07-14-outcome-aware-learning.md).
...
The fresh post-merge three-repository replay then produced one independently
verified BetterFeedback draft, zero-token weak-signal filtering for
suckscancer.com, and an honest Night Shift policy rejection; see
[`2026-07-14-post-merge-portfolio-replay.md`](proofs/2026-07-14-post-merge-portfolio-replay.md).
...
The merged feedback-review linkage proof is recorded in
[`2026-07-14-feedback-review-linkage.md`](proofs/2026-07-14-feedback-review-linkage.md).
The order-independent reconciliation proof is recorded in
[`2026-07-14-review-feedback-reconciliation.md`](proofs/2026-07-14-review-feedback-reconciliation.md).
The coverage prefilter proof is recorded in
[`2026-07-14-coverage-prefilter.md`](proofs/2026-07-14-coverage-prefilter.md).
...
| Repository prioritization | 94 | Live 30-day ranking reduces stale branch and green-draft noise, guarantees the explicit primary repo a selected slot, protects saved priority rows when capacity allows, accepts validated user `owner/repo` priorities, recognizes failed Status Contexts, and applies a real saved useful vote to the canonical GitHub repo identity. Three fresh three-repo cycles ranked the repo with failing checks first, explained each selection reason, and kept the current project in the portfolio. The latest clean-main Mac-only replay ranked BetterFeedback first for three failed checks and recorded the outcome reason for every selected repo. A dedicated ranking proof now shows repeated five-repo selection and shuffled-input stability; consistent accepted-value correlation across more runs is not proven. | Measure that selected repos consistently match accepted user value across repeated overnight runs. |
| GitHub usefulness | 94 | PR, issue, failed-workflow, and recent-repo signals feed queues. Live ranking distinguishes actionable failed/review work from stale branch and green-draft volume; failed Status Contexts now route to review and exact failed hosted checks appear in the publication artifact and morning brief. Portfolio signal ingestion now selects the newest workflow run by `updatedAt` even when GitHub returns history out of order, preventing an older failure from masking a newer pass. The latest no-work portfolio brief also names the failed-check, PR, and issue counts it inspected instead of hiding them behind a generic message. Night Shift PRs #111 and #120 passed Ubuntu/macOS CI and merged. BetterFeedback draft PR #491 remains safely open from an exact SHA with two explicitly failed Vercel checks. Repeated accepted hosted outcomes across owned repos remain missing. | Correlate selected work to accepted PR/issue outcomes across owned repos, including a passing hosted draft. |
| Portfolio discovery | 94 | Fresh authenticated discovery selected and cached three owner-validated private repos, prepared all three checkouts, rejected symlink/path-escape/foreign-owner cases, and supports validated explicit priorities plus quiet hours. Required saved-priority and explicit-primary repos now receive a GitHub signal scan slot even when they fall outside the normal ranked window; primary matching is case-insensitive and its duplicate-fallback regression is recorded in `docs/proofs/2026-07-14-primary-slug-case.md`. Two follow-up live cycles completed a child batch for each discovered repo and kept the current project in the portfolio; the latest no-work cycle recorded honest signal counts for all three. Varied-account and repeated accepted-value behavior remains unproven. | Prove varied-account behavior and correlate discovery to accepted outcomes. |
...
| Windows compute | 95 | The live LAN `qwen3-coder:30b` worker was model-list verified at `192.168.7.201:11434`, then produced a real BetterFeedback one-file behavioral-test draft through bounded Windows calls. Baseline and post-patch `npm run test:unit:vitest` checks passed in an isolated sandbox, the semantic invocation contract passed, and the temporary worktree was removed. The fresh [explicit-lane proof](proofs/2026-07-14-explicit-lane-disable.md) also ran a real Windows-only shift with Mac probes skipped and no local worker process. Earlier live Windows repair and prompt-injection runs remain separately recorded. | Preserve this proof while adding thermal/resource feedback and more varied real-repo repairs. |
| Efficiency | 93 | Weak portfolio work can be filtered to zero tokens. Task fingerprints now use the latest commit touching the candidate files, so docs-only commits do not reopen the same file-level task; pinned PR and CI revisions remain authoritative. Explicit coverage goals now tolerate ordinary verbose wording while remaining bounded and action-specific; the regression is recorded in `docs/proofs/2026-07-14-verbose-coverage-intent.md`. The real current-main replay in `docs/proofs/2026-07-14-task-revision-dedupe.md` shows cycle 1 verifying the cleanup task once and cycle 2 moving to a different grounded task at the same file-scoped revision, without reopening cleanup. The fresh launchd-style portfolio proof visited three repos, spent zero model calls on two weak repos, and produced one verified local draft at 3,472 estimated tokens. Earlier local replays recorded verified drafts at 3,428 and 3,469 estimated tokens and now persist `tokens_per_verified_draft` instead of treating a `MAYBE` as success. The clean three-repo Mac-only replay spent zero model tokens on two weak/no-work repos and 3,413 tokens on one safely rejected candidate. Earlier explicit-goal runs produced verified candidates in about 3,204 and 3,363 estimated local tokens, and the fresh BetterFeedback recovery produced a verified local draft in 3,480 tokens after a 3,516-token rejected attempt. Blank-home unchanged-revision repeats ran with zero model calls and zero-token reports. The bounded Python source-evidence replay in `docs/proofs/2026-07-14-bounded-python-source-evidence.md` used one local call, 3,451 estimated tokens, zero correction calls, and zero rejects after the merged evidence-boundary fix. The clean validator/log-noise replay in `docs/proofs/2026-07-14-validator-and-log-noise-proof.md` used one local call, 3,585 estimated tokens, zero correction calls, zero rejects, a validated MAYBE, and no leaked `SyntaxWarning` output. A fresh three-run Swift replay also stayed fail-closed on citation mismatches at 11,561, 11,646, and 11,788 estimated tokens; no accepted outcome was counted. This is still a small sample, not a full-run accepted-outcome bound. | Prove strong tokens-per-accepted-outcome across full overnight runs with revision caching. |
...
| Draft PR creation | 92 | Fresh publication from exact-main SHAs opened Night Shift PRs #111 and #120 plus BetterFeedback draft PR #491. #111 and #120 passed Ubuntu and macOS CI, were reviewed, and merged as `65513f6` and `a07abbc`. #491 was confirmed draft and locally reverified; the publication path records its two failed Vercel checks instead of implying hosted success. The new read-only `reconcile-drafts` path refreshes current draft and hosted-check evidence for previously opened drafts with atomic local persistence, while preserving unknown results on GitHub read failure. Repeated independently useful PRs across varied repos remain missing. | Produce independently useful tested draft PRs across varied repos during unattended runs without duplicate churn and with passing hosted checks. |
