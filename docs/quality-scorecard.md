...
| Minimal setup | 80 | Live interactive and `--yes` setup require no repo path, URL, model name, or GitHub selection. Real `trust-repo` runs automatically proved ownership, selected one command, preflighted it, saved compatible Python approval, and clearly kept incompatible Draft analysis-only. | Repeat on fresh macOS/Linux users and prove clear recovery when no eligible repo or AI is available. |
...
| Test coverage | 95 | All 181 tests and package/install checks pass on the host; the complete suite also passes in the real no-network Docker VM. Coverage now includes every portfolio signal cap, stale-branch filtering, primary-repo retention, command preflight, candidate binding, publication, and cleanup paths. | Preserve this gate while adding interrupted-controller and scheduler integration cases. |
