...
| First-run UX | 68 | A live blank-home, no-current-repo `--yes --setup-only` run completed with zero questions, automatically selected an owned GitHub repo, and showed safe defaults; interactive one-consent comprehension is not yet observed. | Observe new users completing interactive setup with at most one consent and no endpoint knowledge. |
...
| Minimal setup | 68 | Live setup from an ordinary folder required no repo path, URL, model name, or GitHub selection; it chose mac-only, github-recent, Normal, and eight hours. | Repeat on fresh macOS/Linux users and prove clear recovery when no eligible repo or AI is available. |
...
| Test coverage | 90 | 119 tests now cover reporting plus setup precedence, GitHub fallback, owner/cache validation, read-only dry runs, repeat onboarding, invalid repos, and copied installs. | Real provider, interrupted-controller, scheduler, and GitHub-write integration tests. |
