...
| GitHub usefulness | 78 | PR, issue, failed-workflow, and recent-repo signals feed queues. The live Draft replay turned open issue #38 into an exact one-file review candidate while filtering its parent release tracker. | Correlate selected work to accepted PR/issue outcomes across owned repos. |
...
| Test coverage | 95 | All 178 tests and package/install checks pass on the host; the complete suite also passes in the real no-network Docker VM. Coverage includes command-level preflight save/block control, candidate binding, external-profile metadata, spoof/tamper and symlink rejection, publication, and cleanup paths. | Preserve this gate while adding interrupted-controller and scheduler integration cases. |
